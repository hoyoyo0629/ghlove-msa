 ...
 * 회원 ID로 바꿔치기만 하면 누구나 남의 기부내역/포인트/관심지자체를 보고 조작할 수 있는
 * IDOR 취약점이었다. 이제는 이 쿠키의 서명이 유효할 때만(=member 서비스에서 실제로 로그인한
 * 사람) 그 안의 userId를 신뢰한다 - 쿼리파라미터는 더 이상 신원 증명 수단으로 쓰지 않는다.
 */
@Component
@Slf4j
