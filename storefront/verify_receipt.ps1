param(
    [string]$OutDir = "C:\Users\GHLOVE~1\AppData\Local\Temp\claude\storefront-shots\receipt"
)

New-Item -ItemType Directory -Force -Path $OutDir | Out-Null
$profileDir = Join-Path $env:TEMP "edge-cdp-profile-receipt"
if (Test-Path $profileDir) { Remove-Item -Recurse -Force $profileDir -ErrorAction SilentlyContinue }

$edge = "C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe"
$proc = Start-Process -FilePath $edge -ArgumentList "--headless=new","--disable-gpu","--remote-debugging-port=9349","--user-data-dir=$profileDir","about:blank" -PassThru

$ready = $false
for ($i=0; $i -lt 30; $i++) {
    try { Invoke-RestMethod -Uri "http://127.0.0.1:9349/json/version" -TimeoutSec 1 | Out-Null; $ready = $true; break } catch { Start-Sleep -Milliseconds 500 }
}
if (-not $ready) { Write-Error "CDP not ready"; exit 1 }

$tab = Invoke-RestMethod -Uri "http://127.0.0.1:9349/json/new?about:blank" -Method PUT
...
Nav-And-Wait "http://localhost:5173/mypage/receipts" 2
Shot "01-receipt-list"

$checkFirstTwo = Eval "(function(){ var cbs = document.querySelectorAll('.result-row input[type=checkbox]'); if(cbs.length < 2) return 'NOT_ENOUGH:' + cbs.length; cbs[0].click(); cbs[1].click(); return 'checked:' + cbs[0].value + ',' + cbs[1].value; })()"
Write-Output "CHECK TWO ROWS: $checkFirstTwo"
Start-Sleep -Milliseconds 300

$viewClick = Eval "(function(){ var btns = document.querySelectorAll('.table-top button'); if(btns.length<1) return 'NOT_FOUND'; btns[0].click(); return 'clicked'; })()"
Write-Output "VIEW CERT CLICK: $viewClick"
Start-Sleep -Seconds 1
Shot "02-certificate-modal-front"

$flipClick = Eval "(function(){ var card = document.getElementById('cardContainer'); if(!card) return 'NOT_FOUND'; card.click(); return 'clicked'; })()"
Write-Output "FLIP CARD CLICK: $flipClick"
Start-Sleep -Milliseconds 800
Shot "03-certificate-modal-back"

Nav-And-Wait "http://localhost:5173/mypage/honor-certificates" 2
$goEstimateClick = Eval "(function(){ var links = Array.from(document.querySelectorAll('a')); var link = links.find(function(a){ return a.getAttribute('href') === '/mypage/tax-credit-estimate'; }); if(!link) return 'NOT_FOUND'; link.click(); return 'clicked'; })()"
Write-Output "GO TAX CREDIT ESTIMATE: $goEstimateClick"
Start-Sleep -Seconds 1
Shot "04-tax-credit-estimate"

Nav-And-Wait "http://localhost:5173/honor" 2
Shot "05-tax-credit-guide"
